...
import { useMyPlaceQuery } from "@myPlace/hooks/useMyPlaceQuery";
...
function MyPlacePage() {
  const { data: myPlaces = [], isLoading, isError } = useMyPlaceQuery();
  ...
  return (
    <div>
      ...
      <div>
        ...
        {!isLoading && isError && <div>오류: {isError}</div>}
        {!isLoading && !isError && myPlaces.length === 0 && (
          <div>플레이스가 없습니다.</div>
        )}
        {!isLoading && !isError && myPlaces.length > 0 && (
